...
char cbuf[cBUFF_SIZE] = {'\0'};
char *cbuf_ptr = cbuf;

Acc_Proc gait_proc[fBUFF_SIZE];
...
int16_t spim_write_reg(uint8_t isChA, uint8_t reg_addr, int8_t *data, int16_t data_len)
{
	// printf("into %s-%d\r\n", __func__, __LINE__);
	uint8_t reg_buf[2];
	// uint8_t dev_addr;
...
int16_t SetBaudrate(uint8_t interface, uint8_t isChA, uint32_t baudrate)
{
	...
	if (interface == SC16IS750_PROTOCOL_I2C)
	{
		if ((i2cm_read_reg(isChA, SC16IS750_REG_MCR) & 0x80) == 0)
		{ // if prescaler==1
			printf("into %s-%d\r\n", __func__, __LINE__);
			prescaler = 1;
		}
	...
	{
		if ((spim_read_reg(isChA, SC16IS750_REG_MCR) & 0x80) == 0)
		{ // if prescaler==1
			printf("into %s-%d\r\n", __func__, __LINE__);
			prescaler = 1;
		}
...
void UART_ReadBytes_customized(uint8_t interface)
{
	// printf("into %s-%d\r\n", __func__, __LINE__);
	int8_t tmp;
	uint16_t data_len;
		...
		if (isChA == 1 && rx_a_int_flag == 1)
		{
			...
			else if (interface == SC16IS750_PROTOCOL_SPI)
			{
				while ((tmp = spim_read_reg(CH_A, SC16IS750_REG_RXLVL)) != 0)
				{
					...
					// memcpy(resp_buf + recv_a_index, recv_a_buf + read_index, tmp);
					memcpy(cbuf_ptr, recv_a_buf + read_index, tmp);
					cbuf_ptr += tmp;
...
uint8_t checkGPIO6State(void)
{
	uint8_t gpio_value = GPIOGetPinState(SC16IS750_PROTOCOL_SPI, CH_A, GPIO6);

	if (gpio_value == 1)
	{
		// printf("GPIO 6 to 'HIGH'!\r\n");
		return 1;
	}
	// printf("GPIO 6 to 'LOW'!\r\n");
	return 0;
}

Acc_Proc *ReadFromUart_ParseToFloat(uint8_t interface)
{
	printf("into %s-%d\r\n", __func__, __LINE__);
	// int8_t send_buf[64];
	...
	while (1)
	{
		if (int_flag == 1)
		{
			// memset(resp_buf, '\0', sizeof(resp_buf));
			// memset(send_buf, '\0', sizeof(send_buf));
			UART_ReadBytes_customized(interface);
			// printf("From Arduino UART : %s\r\n", resp_buf);
			// len += sprintf(cbuf + len, "%s", resp_buf);
			// printf("large buff : %s\r\n", cbuf);

			// unsigned int count_from_nRF52 = 1;
			// printf("ret: %2d 52: %d\r\n", ret, count_from_nRF52);
			// if (count_from_nRF52 != count)
			// {
			// 	printf("stop, wrong!!! %d vs %d\r\n", count_from_nRF52, count);
			// 	board_delay_ms(100000);
			// }
			// count += 1;
...
void parseStrToFloat(void)
{
	// i_from_nRF52: 2B + x: 2B + y: 2B + z: 2B = 8B
	uint16_t i_from_nRF52;
	uint8_t rx_buf[6];
	float x, y, z;
	char *curr_ptr = cbuf;
	...
	curr_ptr += 1;
	char *start_ptr = curr_ptr;

	// check CRC again
	uint8_t xorResultBef = 0x00;
	for (int i = 0; i < fBUFF_SIZE * 8; i++)
	{
		// printf("%s%02x%s-", i % 8 == 0 ? "|" : "", curr_ptr[i], (i - 1) % 8 == 0 ? "|" : "");
		xorResultBef ^= start_ptr[i];
	}
	printf("Offset: %d xOR: 0x%x\r\n", start_ptr - cbuf, xorResultBef);

	printf("Parse data start~\r\n");
	for (int i = 0; i < fBUFF_SIZE; i++)
	{
		// printf("curr ptr: %x\r\n", curr_ptr);
		memcpy(&i_from_nRF52, curr_ptr, 2);
		if (i_from_nRF52 != i)
		{
		...
		memcpy(rx_buf, curr_ptr + 2, 6);
		acc_cal(&x, &y, &z, rx_buf);
		// printf("" NRF_LOG_FLOAT_MARKER " " NRF_LOG_FLOAT_MARKER " " NRF_LOG_FLOAT_MARKER "\r\n", NRF_LOG_FLOAT(x), NRF_LOG_FLOAT(y), NRF_LOG_FLOAT(z));
	...
	// check CRC again
	uint8_t xorResultAft = 0x00;
	for (int i = 0; i < fBUFF_SIZE * 8; i++)
	{
		// printf("%s%02x%s-", i % 8 == 0 ? "|" : "", curr_ptr[i], (i - 1) % 8 == 0 ? "|" : "");
		xorResultAft ^= start_ptr[i];
	}
	if (xorResultBef != xorResultAft)
	{
		printf("XOR Error!\r\n");
