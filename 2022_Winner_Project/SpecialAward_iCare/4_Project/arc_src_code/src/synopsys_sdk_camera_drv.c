...
volatile uint32_t g_tick_sensor_std = 0, g_tick_sensor_stream = 0,
                  g_tick_sensor_toggle = 0;
...
void app_get_xshutdown(WE1AppCfg_GPIO_e app_gpio,
                       CIS_XHSHUTDOWN_INDEX_E *gpio_mux);
void app_print_cfg();
...
void app_set_dp_mclk_src() {
  ...
  } else if (app_cust_config.we1_driver_cfg.dp_clk_mux ==
             WE1AppCfg_DP_CLK_Mux_XTAL) {
    hx_drv_dp_set_dp_clk_src(DP_CLK_SRC_XTAL_24M_POST);
  }
  if (app_cust_config.we1_driver_cfg.mclk_clk_mux ==
      WE1AppCfg_MCLK_CLK_Mux_XTAL) {
    hx_drv_dp_set_mclk_src(DP_MCLK_SRC_INTERNAL, DP_MCLK_SRC_INT_SEL_XTAL);
  } else if (app_cust_config.we1_driver_cfg.mclk_clk_mux ==
             WE1AppCfg_MCLK_CLK_Mux_RC36M) {
...
void app_load_cfg_from_flash() {
  ...
  appcfg_header_size +=
      sizeof(app_cust_config.table_header); /**< Table Header */
  appcfg_header_size += sizeof(
      app_cust_config.app_table_info); /**< Application Table information */
  appcfg_header_size +=
      sizeof(app_cust_config.sensor_cfg_table_info); /**< Sensor Configuration
                                                        Table information */
  appcfg_header_size += sizeof(
      app_cust_config.sensor_streamon_table_info); /**< Sensor Stream On Table
                                                      information */
  appcfg_header_size +=
      sizeof(app_cust_config.sensor_streamoff_table_info); /**< Sensor Stream
                                                              Off Table
                                                              information */
  appcfg_header_size += sizeof(
      app_cust_config.we1_table_info); /**< WE-1 Driver Table information */
  appcfg_header_size += sizeof(
      app_cust_config
          .soc_com_table_info); /**< SOC Communication Table information */

  appcfg_len = apptotal_len - appcfg_header_size;
  appcfg_content_sram_addr = appcfg_sram_addr + appcfg_header_size;
  dbg_printf(DBG_LESS_INFO, "header_size=0x%x,apptotal_len=%d,appcfg_len=%d,"
                            "appcfg_sram_addr=0x%x,AppSRAMContent=0x%x\n",
             appcfg_header_size, apptotal_len, appcfg_len, appcfg_sram_addr,
             appcfg_content_sram_addr);

  if (hx_drv_spi_flash_protocol_read(0, flash_appcfg_addr, appcfg_sram_addr,
                                     appcfg_len, 4) != 0) {
    dbg_printf(
        DBG_LESS_INFO,
        "read flash fail flash_appcfg_addr=0x%x,appcfg_sram_addr=0x%x,len=%d\n",
        flash_appcfg_addr, appcfg_sram_addr, appcfg_len);
    app_iot_table_def_config();
  } else {
    dbg_printf(DBG_LESS_INFO, "read flash success "
                              "flash_appcfg_addr=0x%x,appcfg_sram_addr=0x%x,"
                              "len=%d\n",
               flash_appcfg_addr, appcfg_sram_addr, appcfg_len);
    HxGetCRC8((uint8_t *)appcfg_content_sram_addr, appcfg_len, &pu8CRC8_appcfg,
              &pu16CheckSum_appcfg);
    dbg_printf(DBG_LESS_INFO, "pu8CRC8_appcfg=0x%x,pu16CheckSum=0x%x\n",
               pu8CRC8_appcfg, pu16CheckSum_appcfg);

    if ((app_cust_config.table_header.table_crc != pu8CRC8_appcfg) ||
        (app_cust_config.table_header.table_checksum != pu16CheckSum_appcfg) ||
        (app_cust_config.table_header.table_version !=
         APP_CONFIG_TABLE_VERSION)) {
  ...
  algocfg_header_size +=
      sizeof(algo_cust_config.table_header); /**< Table Header */
  algocfg_header_size += sizeof(
      algo_cust_config.algo_table_info); /**< Application Table information */
  algocfg_len = algototal_len - algocfg_header_size;
  algocfg_content_sram_addr = algocfg_sram_addr + algocfg_header_size;
  dbg_printf(DBG_LESS_INFO, "algoheader_size=0x%x,algototal_len=%d,algocfg_len="
                            "%d,algocfg_sram_addr=0x%x,AlgoSRAMContent=0x%x\n",
             algocfg_header_size, algototal_len, algocfg_len, algocfg_sram_addr,
             algocfg_content_sram_addr);

  if (hx_drv_spi_flash_protocol_read(0, flash_algocfg_addr, algocfg_sram_addr,
                                     algocfg_len, 4) != 0) {
    dbg_printf(DBG_LESS_INFO, "read flash fail "
                              "flash_algocfg_addr=0x%x,algocfg_sram_addr=0x%x,"
                              "len=%d\n",
    ...
    dbg_printf(DBG_LESS_INFO, "read flash success "
                              ...
                              "len=%d\n",
               flash_algocfg_addr, algocfg_sram_addr, algocfg_len);
    HxGetCRC8((uint8_t *)algocfg_content_sram_addr, algocfg_len,
              &pu8CRC8_algocfg, &pu16CheckSum_algocfg);
    dbg_printf(DBG_LESS_INFO,
               "pu8CRC8_algocfg=0x%x,pu16CheckSum_algocfg=0x%x\n",
               pu8CRC8_algocfg, pu16CheckSum_algocfg);
    if ((algo_cust_config.table_header.table_crc != pu8CRC8_algocfg) ||
        (algo_cust_config.table_header.table_checksum !=
         pu16CheckSum_algocfg) ||
        (algo_cust_config.table_header.table_version !=
         ALGO_CONFIG_TABLE_VERSION)) {
...
void app_init_flag() {
  ...
  g_image_size = app_cust_config.we1_driver_cfg.hw5x5_cfg.hw55_in_width *
                 app_cust_config.we1_driver_cfg.hw5x5_cfg.hw55_in_height;
#else
  g_image_size = app_cust_config.we1_driver_cfg.jpeg_cfg.enc_width *
...
void synopsys_camera_init(void) {
  ...
  app_get_xshutdown(app_cust_config.we1_driver_cfg.xshutdown_pin_sel,
                    &g_xshutdown_pin);
  ...
  sensordplib_set_xDMA_baseaddrbyapp(g_wdma1_baseaddr, g_wdma2_baseaddr,
                                     g_wdma3_baseaddr);
  sensordplib_set_jpegfilesize_addrbyapp(g_jpegautofill_addr);
  // Power Plan
  hx_drv_pmu_set_ctrl(PMU_PWR_PLAN,
                      app_cust_config.we1_driver_cfg.pmu_powerplan);
...
static int spi_write(uint32_t addr, uint32_t size,
                     SPI_CMD_DATA_TYPE data_type) {
#ifndef SPI_MASTER_SEND
...
void synopsys_camera_start_capture(void) {
  ...
  hx_dplib_evthandler_register_cb(synopsys_camera_evt_handler,
                                  SENSORDPLIB_CB_FUNTYPE_DP);
...
uint8_t cis_raw_algo_check(uint32_t img_addr) {
  ...
  if (g_xdma_abnormal != 0) {
    ...
    sensordplib_stop_swreset_WoSensorCtrl();
    dbg_printf(DBG_APP_PRINT_LEVEL,
               "abnormal re-setup path cur_frame=%d,acc=%d,event=%d\n",
  ...
  if ((g_wdt1_timeout == 1) || (g_wdt2_timeout == 1) || (g_wdt3_timeout == 1)) {
    ...
    dbg_printf(DBG_APP_PRINT_LEVEL,
               "wdma1_fin=%d,wdma2_fin=%d,wdma3_fin=%d,rdma_fin=%d\n",
  ...
  if (g_inp1bitparer_abnormal != 0) {
    ...
    if (g_hw5x5jpeg_err_retry_cnt < MAX_HW5x5JPEG_ERR_RETRY_CNT) {
      ...
      dbg_printf(DBG_APP_PRINT_LEVEL,
                 "HW5x5JPEG 1bitparser Err retry overtime\n");
      app_iot_facedetect_systemreset();
    }
  }
  ...
  // Frame ready
  if (g_frame_ready == 1) {
    // tflitemicro_algo_run(img_addr,
    // app_cust_config.we1_driver_cfg.jpeg_cfg.enc_width,
    // app_cust_config.we1_driver_cfg.jpeg_cfg.enc_height, &algo_result);
...
int app_sensor_streaming() {
  ...
  // stream on
  if (hx_drv_cis_setRegTable(
          app_cust_config.sensor_streamon_cfg.sensor_stream_cfg,
...
int app_sensor_strobe_on() {
  if (app_cust_config.sensor_table_cfg.sensor_id ==
      SENSORDPLIB_SENSOR_HM0360_MODE1) {
    dbg_printf(DBG_LESS_INFO, "strobe on\n");
    hx_drv_sensorctrl_set_MCLKCtrl(SENSORCTRL_MCLKCTRL_NONAOS);
    app_sensor_xsleep_ctrl(SENSORCTRL_XSLEEP_BY_CPU, 1);
    // strobe on
    if (hx_drv_cis_setRegTable(
            app_cust_config.sensor_strobeon_cfg.sensor_strobe_on_cfg,
            app_cust_config.sensor_strobeon_cfg.active_cfg_cnt) !=
        HX_CIS_NO_ERROR) {
...
// FOR Non-AOS need streaming agaign, Toggle stream and standby should over 1
// frame
...
int app_sensor_standby() {
  ...
  // stream off
  if (hx_drv_cis_setRegTable(
          app_cust_config.sensor_streamoff_cfg.sensor_off_cfg,
...
int app_config_sensor_WE1_rx(uint8_t sensor_init_required,
                             uint8_t sensor_strobe_req) {
  if (sensor_init_required == 1) {
    app_config_sensor_all();
  }

  if (sensordplib_set_sensorctrl_inp(
...
int app_config_sensor_all() {
  app_sensor_xsleep_ctrl(SENSORCTRL_XSLEEP_BY_CPU, 1);
  // not need set streaming mode keep in standby, it will config streaming mode
  // in sensor_dplib
  if (hx_drv_cis_setRegTable(app_cust_config.sensor_table_cfg.sensor_cfg,
...
void app_rx_set_wlcsp38_sharepin() {
  ...
  if (app_cust_config.we1_driver_cfg.chip_package ==
      WE1AppCfg_CHIP_Package_WLCSP38) {
    if ((app_cust_config.sensor_table_cfg.sensor_id ==
         SENSORDPLIB_SENSOR_HM11B1_LSB) ||
        (app_cust_config.sensor_table_cfg.sensor_id ==
         SENSORDPLIB_SENSOR_HM11B1_MSB)) {
      hx_drv_sensorctrl_set_wlcsp_sharepin(SENSORCTRL_WLCSP_SHAREPIN_HM11B1);
    } else if ((app_cust_config.sensor_table_cfg.sensor_id ==
                SENSORDPLIB_SENSOR_HM01B0_1BITIO_LSB) ||
               (app_cust_config.sensor_table_cfg.sensor_id ==
                SENSORDPLIB_SENSOR_HM01B0_1BITIO_MSB)) {
...
void synopsys_camera_down_scaling(uint8_t *input_image, uint32_t input_width,
                                  uint32_t input_height, uint8_t *output_image,
  ...
  for (height_i1 = 0; height_i1 < output_height; height_i1++) {
    ...
    for (width_i1 = 0; width_i1 < output_width; width_i1++) {
      pixel_buf = 0;
      input_ptr_index1 =
          (height_i1 * height_step * input_width) + (width_i1 * width_step);
