...
const uint8_t OledFontTable[] = {
    ...
    0x41, 0x00, // [
    0x55, 0xAA, 0x55,
    0xAA, 0x55, // Backslash (Checker pattern)
    0x00, 0x41, 0x41,
    0x7F, 0x00, // ]
    0x04, 0x02, 0x01,
    0x02, 0x04, // ^
 ...
 * description  :This function is used to initialize the OLED in the normal
 mode.
                After initializing the OLED, It clears the OLED and sets the
 ...
 * description  : This function sends a character to be displayed on LCD.
                  Any valid ascii value can be passed to display respective
 character
...
void OLED_DisplayChar(int8_t ch) {
  ...
  if (ch != '\n') { /* TODO */
    ...
    for (i = 0; i < 5; i++)
      oledSendData(OledFontTable[index + i]); /* Get the data to be displayed
                                                 for LookUptable*/
 ...
 * description  :
               This function is used to display the ASCII string on the lcd.
                 1.The ptr_stringPointer_u8 points to the first char of the
 string
                    and traverses till the end(NULL CHAR)and displays a char
 each time.
...
// Same as OLED_DisplayString, but will Flush trailing characters after string
// in same col
void OLED_DisplayString_Flush(uint8_t *ptr) {
  int i = 0;
  while (*ptr) {
    OLED_DisplayChar(*ptr++);
    ++i;
...
// A page is a line in SSD1306
void OLED_Clear_Col(uint8_t Col) {
  OLED_SetCursor(Col, 0);
...
void oledSendCommand(uint8_t cmd) {
  ...
  hx_drv_i2cm_write_data(USE_SS_IIC_X, SSD1306_ADDRESS, &data_write[0], 1,
                         &data_write[1], 1);
}
...
void oledSendData(uint8_t cmd) {
  ...
  hx_drv_i2cm_write_data(USE_SS_IIC_X, SSD1306_ADDRESS, &data_write[0], 0,
                         &data_write[0], 2);
}
