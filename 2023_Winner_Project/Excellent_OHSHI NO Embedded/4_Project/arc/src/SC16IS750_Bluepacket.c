...
//#include "rb8762cjf_Bluepacket.h"
...
int16_t UartInit(uint8_t interface)
{
	...
	//RES for RB8762C-RA
//	GPIOSetPinMode(CH_A, GPIO2, OUTPUT);
//	GPIOSetPinState(CH_A, GPIO2, HIGH);
...
int16_t GetBLEInfo()
{
	...
	memset(send_buf, '\0', sizeof(send_buf));
	sprintf(send_buf, "\r\n\r\n");
	i2cm_write_reg(CH_A, SC16IS750_REG_THR, send_buf, 4);
	board_delay_ms(100);

	//Send AT
	memset(send_buf, '\0', sizeof(send_buf));
	sprintf(send_buf, AT_CMD);
	i2cm_write_reg(CH_A, SC16IS750_REG_THR, send_buf, strlen(send_buf));
	while(int_flag==0){
		board_delay_ms(10);
	}
	...
	//Read BT ADDR	
	memset(send_buf, '\0', sizeof(send_buf));
	sprintf(send_buf, AT_CMD_READ_BT_ADDR);
	i2cm_write_reg(CH_A, SC16IS750_REG_THR, send_buf, strlen(send_buf));
	while(int_flag==0){
		board_delay_ms(10);
	}
	...
	//BT NAME
	memset(send_buf, '\0', sizeof(send_buf));
	sprintf(send_buf, AT_CMD_READ_BT_NAME);
	i2cm_write_reg(CH_A, SC16IS750_REG_THR, send_buf, strlen(send_buf));
	while(int_flag==0){
		board_delay_ms(10);
	}
	...
	//BT Baudrate
	memset(send_buf, '\0', sizeof(send_buf));
	sprintf(send_buf, "%s", AT_CMD_READ_BT_BAUDRATE);
	i2cm_write_reg(CH_A, SC16IS750_REG_THR, send_buf, strlen(send_buf));
	while(int_flag==0){
		board_delay_ms(10);
	}
	...
	//BT Version
	memset(send_buf, '\0', sizeof(send_buf));
	sprintf(send_buf, AT_CMD_READ_VERSION);
	i2cm_write_reg(CH_A, SC16IS750_REG_THR, send_buf, strlen(send_buf));
	while(int_flag==0){
		board_delay_ms(10);
	}
