...
def output_to_target(output):
    # Convert model output to target format [batch_id, class_id, x, y, w, h, conf , 再加一個dist]
    targets = []
    for i, o in enumerate(output):
        #for *box, conf, cls in o.cpu().numpy():
            #targets.append([i, cls, *list(*xyxy2xywh(np.array(box)[None])), conf])
        for *box, conf, dist, cls in o.cpu().numpy():
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16): 
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            '''Change by Cheng'''
            distances = image_targets[:, 6] if labels else image_targets[:, 7]
            ...
            for j, box in enumerate(boxes.T):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    label = '%s %.1f' % (cls,distances[j]) if labels else '%s %.1f %.1fm' % (cls, conf[j], distances[j])
