...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    Returns:
         list of detections, on (n,6) tensor per image [xyxy, conf,**dist** , cls]
    """
    #此處input prediction的形式會是(?,很大,nc+5+1) (x,y,w,h,conf,dist,...(class))
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x = x[xc[xi]]  # confidence #找出conf夠大的那些元素
        ...
        if labels and len(labels[xi]):
            ...
            v[range(len(l)), l[:, 0].long() + 6] = 1.0  # v也會跟prediction一樣的形式
            x = torch.cat((x, v), 0)
        ...
        # Detections matrix nx(6+1) (xyxy, conf, cls)
        if multi_label and 0:
            #i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T
            i, j = (x[:, 6:] > conf_thres).nonzero(as_tuple=False).T

            #x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
            x = torch.cat((box[i], x[i, j + 6, None], j[:, None].float()), 1)
            '''Problem here the output x.shape is (?, 6)'''

        else:  # best class only
            #conf, j = x[:, 5:].max(1, keepdim=True)
            conf, j = x[:, 6:].max(1, keepdim=True)
            x = torch.cat((box, conf, x[:, 5:6], j.float()), 1)[conf.view(-1) > conf_thres]
