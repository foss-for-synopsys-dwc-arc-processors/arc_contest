...
{
    oledSendCommand(0xB0 | page); // Set the page address
    oledSendCommand(0x00 | (cursorPosition & 0x0F)); // Set the lower column address
    oledSendCommand(0x10 | (cursorPosition >> 4)); // Set the higher column address
}
...
{    
    ...
    for(oled_clean_page = 0 ; oled_clean_page < 8 ; oled_clean_page++) {
        for(oled_clean_col= 0 ; oled_clean_col < 128 ; oled_clean_col ++) {
            OLED_SetCursor(oled_clean_col, oled_clean_page);
            oledSendData(0);
        }
    }
}
...
{	
    ...
    if(location == 'L')
        for(oled_clean_page = 0 ; oled_clean_page < 8 ; oled_clean_page++) {
            ...
            for(oled_clean_col = 0 ; oled_clean_col < 64 ; oled_clean_col ++) {
                oledSendData(0);
            }
        }
    else if(location == 'R')
        for(oled_clean_page = 0 ; oled_clean_page < 8 ; oled_clean_page++) {
            OLED_SetCursor(oled_clean_page, 64); // 將起始位置設為64列，也就是正中間
            for(oled_clean_col = 0 ; oled_clean_col < 64 ; oled_clean_col ++) {
                oledSendData(0);
            }
        }
...
void displayCircle() {
    ...
    for (row = 0; row < 8; row++) {
        OLED_SetCursor(row, 43);

        for (col = 0; col < 8; col++) {
            OLED_DisplayChar(image[row][col]);
        }
    }
    // OLED_SetCursor(7, 35);
    // OLED_DisplayString("Safe Driver");
}
...
void displayCross() {
    ...
    for (row = 0; row < 7; row++) {
        ...
        for (col = 0; col < 8; col++) {
            OLED_DisplayChar(cross[row][col]);
        }
    }
    // OLED_SetCursor(7, 9);
    // OLED_DisplayString("Put your hands back");
}