...
uint16_t recv_a_index=0;
uint8_t resp_buf[8192];
// float rssi_model_buf[60];
// float acc_model_buf[60];
uint8_t recv_a_buf[8192];
...
int UART_ReadBytes(uint8_t interface)
{
	//printf("into %s-%d\r\n", __func__, __LINE__);
	int8_t tmp;
	char *pch,*pch1;
		...
		if(isChA == 1 && rx_a_int_flag ==1){
			if(interface == SC16IS750_PROTOCOL_I2C){
				...
				while((tmp = spim_read_reg(CH_A, SC16IS750_REG_RXLVL))!=0){
					...
					// printf("tmp: %d\r\n",tmp);
					// printf("recv_a_index: %d\r\n",recv_a_index);
					if (recv_a_buf[0]==0){
						// printf("empty allert\r\n");
						break;
					}
					memcpy(resp_buf+recv_a_index, recv_a_buf+read_index, tmp);
					pch = strstr(recv_a_buf, "!");
					...
					// 	printf("not found\n");
					// }			
					//memcpy(resp_buf, recv_a_buf, tmp);
					//printf("%s", resp_buf);
					//p = strtok (resp_buf,",");
					//memcpy(rssi_model_buf+recv_a_index, p, sizeof(resp_buf));
...
void test_UART_Read(float *rssi_model_buf,float * acc_model_buf) {
	printf("into %s-%d\r\n", __func__, __LINE__);
	int8_t send_buf[64];
	int a=0;
	int i,j=0;
	...
	while(1)
	{
		...
		{
			//memset(resp_buf, '\0', sizeof(resp_buf));
			a = UART_ReadBytes(SC16IS750_PROTOCOL_SPI);
			//printf("%s,777\n", resp_buf);
			board_delay_ms(1);
		}
	...
	// printf("%c\r\n",resp_buf[2]);
	// test_int=strtof(temp,NULL)*100;
	// printf("get: %d\r\n",test_int);

	for(i=0;i<8192;i++)
	{
		if(resp_buf[i]!='\0'&& resp_buf[i]!='!')
		{
			if(resp_buf[i]==','||resp_buf[i]=='\r')
			{
				if(flag_write==0) //rssi
				{
					rssi_model_buf[rssi_index]=strtof(temp,NULL);
					//printf("get: %f\r\n",strtof(temp,NULL));
					rssi_index++;
				}
				else //acc
				{
					acc_model_buf[acc_index]=strtof(temp,NULL);
					//printf("get: %f\r\n",strtof(temp,NULL));
					//printf("acc index:%d \r\n",acc_index);
					acc_index++;
				}
	...
	// for(i=0;i<60;i++)
	// {
	// 	printf("rssi : %.1f , ",rssi_model_buf[i]);
	// 	printf("acc : %.2f \r\n",acc_model_buf[i]);
	// }
